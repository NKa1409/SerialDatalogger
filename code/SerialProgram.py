...
def serial_commands():
        print("   'log' - Logs incoming data from serial port\n"
              ...
              "   'show serial' - Prints all serial data to screen (everything visible)\n"
              "\n"
...
while True:
    serial_commands()
    while True:
        cmd = NormalFunctions.get_string_from_user()
        if cmd is None:
            serial_commands()
            continue
        if cmd.lower() == "close":
            ...
            import NormalFunctions
            delimiter = NormalFunctions.get_string_from_user()
            print("What is the second delimiter? \n"
                  "The second delimiter stands in front of the actual value you want "
                  "to be saved.\n"
                  "e.g.: incoming data: 'U1=0.243,U2=1.23,U3=1.002'; ',' is first delimiter, '=' is second delimiter")
            second_delimiter = NormalFunctions.get_string_from_user()
            print("Press 'q' to stop logging data.")
            while True:
                serial_data = serial.serial_readline_to_file_if_line_is_available()
                if not (serial_data is None):
            ...
            to_be_written_to_serial = NormalFunctions.get_string_from_user()
